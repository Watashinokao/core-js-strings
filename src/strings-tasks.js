...
function removeFirstOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    return str
      .slice(0, indexStart)
      .concat(str.slice(indexStart + value.length, str.length));
  }
  return str;
}
...
function findLongestWord(sentence) {
  const word = sentence.split(' ').sort((a, b) => b.length - a.length);
  return word[0];
}
...
function encodeToRot13(str) {
  ...
  function search(index) {
    if (index > 25) {
      return arrAl[index - arrAl.length];
    }
    return arrAl[index];
  }
  const myArr = [];
  arr.map((el) => {
    ...
    if (el === ' ') {
      myArr.push(el);
    } else if (el === el.toLowerCase() && arrAl.includes(el.toUpperCase())) {
      index = arrAl.indexOf(el.toUpperCase());
      newEl = search(index + 13).toLowerCase();
      myArr.push(newEl);
    } else if (el === el.toUpperCase() && arrAl.includes(el.toUpperCase())) {
      index = arrAl.indexOf(el);
